$(document).ready(function () {
    $('.btn_update_image_supplies').click(function (event) {
        event.preventDefault();
        var supplies_id = $("input[name='supplies_id']").val()
        var count = $("img[name='img_asset']").length;
        var image_selector = $("img[name='img_asset']");
        var image_asset = {};
        if (count > 0) {
        ...
        $.ajax({
            url: window.origin + '/supplies/update_image',
            type: "POST",
            data: formData,
            dataType: 'json',
            headers: {
    ...
    $('.btn-confirm').click(function () {
        ...
        $.ajax({
            url: window.origin + '/supplies/show_ajax/' + id,
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                console.log(data)
                $("input[name='supplies_id']").val('')
                $("input[name='name_supplies']").val('')
                $("input[name='user_id']").val('')
                $("textarea[name='description']").val('')
                $('.block').remove()
                $("input[name='supplies_id']").val(id)
                $("input[name='name_supplies']").val(data.data.name)
                $("input[name='user_id']").val(typeof data.data.user_id == undefined ? '' : data.data.user.email)
            },
            error: function () {
                console.log('error')
            }
    ...
    $('.btn_confirm_supplies').click(function (event) {
        event.preventDefault();
        var supplies_id = $("input[name='supplies_id']").val()
        ...
        $.ajax({
            url: window.origin + '/supplies/office_confirm',
            type: "POST",
            data: formData,
            dataType: 'json',
            headers: {
